...
import { serializeEsiJssState, unescapeEsiHtml } from '@uniformdev/esi-jss-ssr';
    ...
    */
    initializei18n(state)
      ...
      .then(({ renderedAppHtml, htmlTemplate }) => {
        ...
        // Determine whether the app markup has ESI tags so we can conditionally perform
        // more post-processing tasks.
        const hasEsiTags = renderedAppHtml.indexOf('<esi:choose>') !== -1;

        // Rendered app HTML may contain escaped characters that will cause ESI parsing/evaluation to fail,
        // so we need to modify the HTML if necessary.
        const appHtml = hasEsiTags ? unescapeEsiHtml(renderedAppHtml) : renderedAppHtml;
        ...
        // Serialize the state and write the string version of our state to the outgoing HTML.
        // NOTE: if our markup contains ESI tags, then we use a custom serializer to serialize
        // the state data to include ESI tags. Otherwise, use the default serializer.
        const serializedState = hasEsiTags
          ? serializeEsiJssState(state)
          : serializeJavascript(state, { isJSON: true });

        html = assertReplace(
          html,
          '<script type="application/json" id="__JSS_STATE__">null',
          `<script type="application/json" id="__JSS_STATE__">${serializedState}`
        );
